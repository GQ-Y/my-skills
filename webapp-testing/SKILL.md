...
**始终首先使用 `--help` 运行脚本**以查看用法。在尝试首先运行脚本并发现绝对需要自定义解决方案之前，不要读取源代码。这些脚本可能非常大，因此会污染您的上下文窗口。它们存在是为了作为黑盒脚本直接调用，而不是被吸收到您的上下文窗口中。
...
- **使用捆绑脚本作为黑盒** - 要完成任务，考虑 `scripts/` 中可用的脚本之一是否可以提供帮助。这些脚本可靠地处理常见、复杂的工作流程，而不会使上下文窗口混乱。使用 `--help` 查看用法，然后直接调用。
...
- **examples/** - 显示常见模式的示例：
  ...
  - `static_html_automation.py` - 使用 file:// URL 用于本地 HTML
