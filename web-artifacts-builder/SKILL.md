...
description: 用于使用现代前端 Web 技术（React、Tailwind CSS、shadcn/ui）创建精心制作的多组件 claude.ai HTML 工件的工具套件。用于需要状态管理、路由或 shadcn/ui 组件的复杂工件 - 不用于简单的单文件 HTML/JSX 工件。
...
**堆栈**：React 18 + TypeScript + Vite + Parcel（打包）+ Tailwind CSS + shadcn/ui

## 设计和样式指南

非常重要：为了避免通常被称为"AI 草率"的情况，避免使用过多的居中布局、紫色渐变、统一的圆角和 Inter 字体。
...
要测试/可视化工件，使用可用工具（包括其他技能或内置工具如 Playwright 或 Puppeteer）。通常，避免提前测试工件，因为它在请求和可以看到完成的工件之间增加了延迟。如果请求或出现问题，在呈现工件后稍后测试。
